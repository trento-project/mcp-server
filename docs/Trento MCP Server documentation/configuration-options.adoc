...
|`--oas-path, -P` |`TRENTO_MCP_OAS_PATH` |`./openapi.json` |Path to the OpenAPI spec file(s). Can be specified multiple times. Server URLs in specs are automatically overwritten with the `--trento-url` value.
...
* `api-documentation` - Checks accessibility of OpenAPI specification files (remote URLs only)
