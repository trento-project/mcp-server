...
## Enable MCP protocol 2026-07-28 stateless mode (no session state, no 'initialize' handshake). Cannot be combined with TRANSPORT=sse.
STATELESS=false
...
|`--stateless, -s` |`TRENTO_MCP_STATELESS` |`false` |Enable MCP protocol 2026-07-28 stateless mode. Cannot be combined with `--transport sse`.
...
[[sec-mcp-stateless-mode]]
== Stateless Mode

The `--stateless` flag (or `STATELESS=true`) switches the `streamable` transport to the
stateless model introduced by MCP protocol version `2026-07-28`. In this mode:

* No `Mcp-Session-Id` is ever issued or read. Every request is handled as an independent,
  self-contained call - there is no `initialize` handshake to establish state ahead of time.
* `GET` and `DELETE` requests to the MCP endpoint return `405 Method Not Allowed`, since there is
  no long-lived stream or session to open/close.
* Older clients that still perform the legacy `initialize` handshake keep working, but the
  handshake itself carries no state across calls - it is answered and immediately forgotten.

[IMPORTANT]
====
Because there is no session to cache a bearer token against, **the client must send the
authentication header (`--header-name`, `Authorization` by default) on every `tools/call`
request**, not just once at connection setup. In the default (non-stateless) mode, the server
caches the token seen during `initialize` and replays it on later calls within that session, so
some clients only send it once; that shortcut does not exist in stateless mode.
====

[TIP]
====
Stateless mode also removes the need for session affinity ("sticky sessions") when running
multiple replicas behind a load balancer. In the default mode, the token cache lives in a single
server process's memory, so `initialize` and subsequent `tools/call` requests for the same client
must land on the same replica. Stateless mode has no such requirement, since every request is
authenticated independently.
====

`--stateless` cannot be combined with `--transport sse`: the HTTP+SSE transport is deprecated by
the MCP specification and is inherently session-based, so it is rejected at startup when
combined with `--stateless`.

[source,console]
----
mcp-server-trento --stateless --trento-url https://prod.trento.io
----
