...
** Follow the xref:../Trento MCP Server documentation/installation.adoc[Installation Guide] to deploy the Trento Server and enable the Trento MCP Server.
...
For MCP Hosts that don't support prompted input, or for testing purposes, you can set your Trento PAT directly in the configuration file.
...
* If the Trento MCP Server is running with `--stateless` enabled (see
  xref:../Trento MCP Server documentation/configuration-options.adoc#sec-mcp-stateless-mode[Stateless Mode]),
  your client must send the authentication header on every tool call, not just once when the
  connection is established - some clients that only send it once may need reconfiguring.
...
* xref:../Trento MCP Server documentation/installation.adoc[Trento MCP Server Installation Guide] - For deploying and configuring the MCP Server.
* xref:../Trento MCP Server documentation/configuration-options.adoc[Configuration Options] - For detailed MCP Server configuration reference.
