...
** Follow the xref:installation.adoc[Installation Guide] to deploy the Trento Server and enable the Trento MCP Server.
...
* Replace `https://trento.example.com/mcp-server-trento/mcp` with the actual URL where your Trento MCP Server is accessible:
  ...
  ** If using Server-Sent Events (SSE) transport instead of the default streamable transport, change the path from `/mcp` to `/sse` - The transport type is configured on the Trento MCP Server using the `TRANSPORT` configuration key or `--transport` flag (see xref:configuration-options.adoc[Configuration Options]).
...
** Refer to the xref:installation.adoc#sec-mcp-troubleshooting[Installation Guide Troubleshooting section] for additional help.
...
* xref:installation.adoc[Trento MCP Server Installation Guide] - For deploying and configuring the MCP Server.
* xref:configuration-options.adoc[Configuration Options] - For detailed MCP Server configuration reference.
