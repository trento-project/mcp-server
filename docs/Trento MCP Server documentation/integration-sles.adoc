...
** Private/on-premises option, such as https://documentation.suse.com/suse-ai/1.0/[SUSE AI Platform] - See the guide on xref:integration-suse-ai.adoc[Integrating the Trento MCP Server with SUSE AI].

. *A running Trento Server installation* with the Trento MCP Server component enabled:
** Follow the xref:installation.adoc[Installation Guide] to deploy Trento Server and enable the Trento MCP Server.
