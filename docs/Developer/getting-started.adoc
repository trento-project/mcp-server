...
To run it in MCP protocol 2026-07-28 stateless mode (see
...
2. **Authentication Problems**: Check that the provided Trento PAT is correct.
3. **Transport Issues**: Try switching between `streamable` and `sse` transports. Note that `--stateless` only works with `streamable`.
